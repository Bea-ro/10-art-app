    ...
    .then((data) => {
      if (typeof data === 'string') {
        ...
        if (itemType === 'artworks') {
          values.image &&
            uploadImage(data._id, token, values.image, artworks, setArtworks)
          setMessage(`${itemType.slice(0, -1)} saved.`)
          const existingAuthor = authors.find(
            (author) => author.name === values.author
          )
          existingAuthor &&
            addArtworksToAuthor(token, data._id, existingAuthor._id, setMessage)
        } else {
...
const uploadImage = async (
  ...
  await fetch(`${process.env.NEXT_PUBLIC_API_URL}/artworks/${id}`, {
    method: 'PATCH',
    headers: {
      Authorization: `Bearer ${token}`
    },
